...
- 🎯 Entry points to execute your program using `python -m <fazy>` or `$ fazy` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/denislour/fazy/branch/main/graph/badge.svg?token=fazy_token_here)](https://codecov.io/gh/denislour/fazy)
[![CI](https://github.com/denislour/fazy/actions/workflows/main.yml/badge.svg)](https://github.com/denislour/fazy/actions/workflows/main.yml)

Awesome fazy created by denislour

## Install

from source
```bash
git clone https://github.com/denislour/fazy fazy
cd fazy
make install
```
...
```bash
❯ fazy --help
Usage: fazy [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ fazy create-user --help
Usage: fazy create-user [OPTIONS] USERNAME PASSWORD
...
```bash
fazy create-user admin admin --superuser
```
...
Run with `fazy run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `fazy create-user`.
...
-----------------------------------------------------
fazy/__init__.py              4      0   100%
fazy/app.py                  16      1    94%
fazy/cli.py                  21      0   100%
fazy/config.py                5      0   100%
fazy/db.py                   10      0   100%
fazy/models/__init__.py       0      0   100%
fazy/models/content.py       47      1    98%
fazy/routes/__init__.py      11      0   100%
fazy/routes/content.py       52     25    52%
fazy/routes/security.py      15      1    93%
fazy/routes/user.py          52     26    50%
fazy/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from fazy.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in fazy/default.toml.

### As environment variables
```bash
export fazy_KEY=value
export fazy_KEY="@int 42"
export fazy_KEY="@jinja {{ this.db.uri }}"
export fazy_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
